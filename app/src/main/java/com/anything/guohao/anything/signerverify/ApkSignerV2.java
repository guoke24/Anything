...
public class ApkSignerV2 {
    ...
    public static ByteBuffer[] sign(
            RandomAccessFile zip,//增加一个参数，适配ZipUtils类
            ByteBuffer inputApk,
        ...
        int eocdOffset = Math.toIntExact(ZipUtils.findZipEndOfCentralDirectoryRecord(zip).second);//修改点
        if (eocdOffset == -1) {
            throw new ApkParseException("Failed to locate ZIP End of Central Directory");
        }
        if (ZipUtils.isZip64EndOfCentralDirectoryLocatorPresent(zip, eocdOffset)) {//修改点

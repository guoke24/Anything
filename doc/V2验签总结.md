...
从此处可以看出，验签的主逻辑分为两步，第一是提取签名信息，第二就是验证签名。
...
3-3处，根据中央区的偏移量，找到并返回签名分块的字节段和偏移量。（文章的前面有对 apk签名分块的格式讲解）
3-4处，从apk签名分块中找到并返回v2签名分块的字节段。（v2签名分块 是 apk签名分块的中的第一个子分块，通过固定ID可以查找出来）
